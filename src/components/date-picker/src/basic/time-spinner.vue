  ...
  <div class="el-time-spinner" :class="{ 'has-seconds': showSeconds }">
    <template v-if="!arrowControl">
      ...
      <el-scrollbar
        ...
        ref="minutes">
        <li
          @click="handleClick('minutes', { value: value, disabled: false })"
          v-for="(value, key) in minutesList"
          :key="key * 5"
          class="el-time-spinner__item"
          :class="{ 'active': key * 5 === minutes, disabled: false }">{{ ('0' + value).slice(-2) }}</li>
      </el-scrollbar>
      <el-scrollbar
        v-show="showSeconds"
...
<script type="text/babel">
  import { getRangeHours, modifyTime } from 'element-ui/src/utils/date-util';
  ...
  export default {
    ...
    methods: {
      ...
      handleScroll(type) {
        ...
        this.modifyDateField(type, value * (type === "minutes" ? 5 : 1));
      },
      ...
      adjustSpinner(type, value) {
        ...
        if (el) {
          el.scrollTop = Math.max(0, (value * this.typeItemHeight(type)) / (type === "minutes" ? 5 : 1));
        }
      },
