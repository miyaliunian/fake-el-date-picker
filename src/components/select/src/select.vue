<template>
  <div class="el-select" :class="[selectSize ? 'el-select--' + selectSize : '']" @click.stop="toggleMenu" v-clickoutside="handleClose">
    <div class="el-select__tags" v-if="multiple" ref="tags" :style="{ 'max-width': inputWidth - 32 + 'px', width: '100%' }">
      ...
      <template v-else>
        <span v-for="item in selected.slice(0, Math.min(selected.length, cardLength))" :key="getValueKey(item)">
          <el-tag :closable="!selectDisabled" :hit="item.hitState" :size="collapseTagSize" disable-transitions type="info" @close="deleteTag($event, item)">
            <span class="el-select__tags-text">{{ item.currentLabel }}</span>
          </el-tag>
          <el-tag v-if="cardLength > selected.length" :closable="false" :size="collapseTagSize" disable-transitions type="info">
            <span class="el-select__tags-text">+ {{ cardLength - selected.length }}</span>
          </el-tag>
        </span></template
      >
